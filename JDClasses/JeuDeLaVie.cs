    ...
    public static class JeuDeLaVieTable
    {
        ...
        public static void GenerateNew(int memoryDistance = 1000, int nbAncientSummaries = 9, bool affichageChangement = false, double probabilite = 0.02, int tailleX = 800, int tailleY = 600, bool staleProof = false, bool verifyStale = false)
        {
            ...
            for (int y = 0; y < tailleY; y++)
            {
                for (int x = 0; x < tailleX; x++)
                {
                    ...
                    int[] cycleMapTableTemp = cycleMapTable1[y][x];
                    cycleMapTableTemp[0] = xZero ? tailleXMinus : xMinus;
                    cycleMapTableTemp[1] = yZero ? tailleYMinus : yMinus;
                    cycleMapTableTemp[2] = xMax ? 0 : xPlus;
                    cycleMapTableTemp[3] = yMax ? 0 : yPlus;
        ...
        public static void generateImage()
        {
            for (int y = 0, yByXtotal = 0; y < _tailleY; y++, yByXtotal += _tailleX)
            {
                ...
                {
                    switch(TableauDeLaVie[ArrayGPS.SwapTablesNew][x, y]) { 
                        case 1:
        ...
        public static void setLife(int x, int y, byte value = 1)
        {
            ...
            {
                cycleMapTable1[rY][rX][IndexStorageSwapNew] = cycleMapTable1[rY][rX][IndexStorageSwapNow] = cycleMapTable1[rY][rX][IndexStorageSwapOld] = value;
                    ...
                    else
                    {
                        ...
                        {                            
                            if (cellSummary > 255)
                            {
                                ...
                                if (tOne[1] != tOne[0])
                                {
                                    tempTableX[IndexStorageSwapNew] = tempTableX[IndexStorageSwapNow] = tZero[0];
                                    intWeight = 255;
